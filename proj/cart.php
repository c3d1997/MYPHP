...
if (!empty($_SESSION['cart'])) {
    ...
    $sql = sprintf("SELECT * FROM products WHERE sid IN (%s)", implode(
        ',',
        $sids
    ));
                ...
                <tbody>
                    ...
                    ?>
                        <tr data-sid="<?= $sid ?>">
                            ...
                            <td>
                                <select class="form-select form-select-sm quantity" style="display:inline-block; width:auto">
                                    <?php for ($i = 1; $i <= 20; $i++) : ?>
...
<script>
    ...
    const calcPrices = () => {
        ...
        trs.each(function() {
            ...
            tr.find('.sub-total').text('$' + dallorCommas(price * quantity)); //顯示小記
